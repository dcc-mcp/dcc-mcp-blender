...
| `blender-light-rig` | lookdev, render, environment | Create reusable light rigs, softboxes, HDRI worlds and rotation animation, grouped light collections, and view-transform settings. | Load after basic lighting when scenes need repeatable studio, environment, or fixed-camera rotating-light LookDev. | Mutating except rig listing and summary. | three point light, softbox, hdri world, animate hdri rotation, light rig, view transform, lighting summary |
...
- Use `blender-lighting` for individual lights and quick world color edits; use `blender-light-rig` for grouped three-point rigs, softboxes, HDRI worlds, fixed-camera environment rotation, and render view-transform coordination.
