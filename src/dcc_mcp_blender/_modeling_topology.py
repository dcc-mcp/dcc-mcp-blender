...
def _topology_exception_receipt(obj, before, exc, operation) -> dict:
    """Report unexpected failures after a destructive mesh operator was invoked."""
    try:
    ...
    return skill_error(
        ...
        "The mesh operator may have mutated the object before verification failed.",
...
def bevel_edges(object_name: str, edge_indices: Sequence[int], width: float, segments: int = 1) -> dict:
    ...
    try:
        ...
        object_mode(bpy)
        after, evidence_error = post_mutation_mesh_state("Bevel", obj, before)
        if evidence_error:
        ...
        if obj is not None and operation_started:
            return _topology_exception_receipt(obj, before, exc, "Bevel")
...
def extrude_faces(
    ...
    try:
        ...
        object_mode(bpy)
        after, evidence_error = post_mutation_mesh_state("Extrude", obj, before)
        if evidence_error:
            return evidence_error
        if not operator_finished(operator_result):
            return skill_error(
        ...
        object_mode_if_available()
        if obj is not None and operation_started:
            return _topology_exception_receipt(obj, before, exc, "Extrude")
...
def inset(
    ...
    try:
        ...
        object_mode(bpy)
        after, evidence_error = post_mutation_mesh_state("Inset", obj, before)
        if evidence_error:
            return evidence_error
        if not operator_finished(operator_result):
            return skill_error(
        ...
        object_mode_if_available()
        if obj is not None and operation_started:
            return _topology_exception_receipt(obj, before, exc, "Inset")
...
def add_edge_loop(object_name: str, edge_indices: Sequence[int], cuts: int = 1) -> dict:
    ...
    try:
        ...
        object_mode(bpy)
        after, evidence_error = post_mutation_mesh_state("Edge loop", obj, before)
        if evidence_error:
            return evidence_error
        if not operator_finished(operator_result):
            return skill_error(
        ...
        object_mode_if_available()
        if obj is not None and operation_started:
            return _topology_exception_receipt(obj, before, exc, "Edge loop")
